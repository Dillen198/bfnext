...
function uuid(): string {
  if (typeof (crypto as Crypto | undefined) !== 'undefined' && typeof crypto.randomUUID === 'function')
...
function HeatmapLayer({ points }: { points: [number, number, number][] }) {
  ...
  useEffect(() => {
    ...
    if (!ref.current) ref.current = (L as any).heatLayer(points, {
      radius: 25, blur: 20, maxZoom: 12, max: 1,
      gradient: { 0.2: '#002200', 0.5: '#006600', 0.8: '#00cc00', 1.0: '#39ff14' }
    }).addTo(map)
...
export default function MapPage() {
  ...
  const handlePlanClick = useCallback((lat: number, lon: number) => {
    if (planMode === 'waypoint') setWaypoints(p => [...p, { id: uuid(), lat, lon }])
    else if (planMode === 'marker') setPlanMarkers(p => [...p, { id: uuid(), lat, lon, type: markerType }])
  ...
  const handleContextMenu = useCallback((lat: number, lon: number) => {
    if (braaStart) {
      setDrawnBraas(p => [...p, { id: uuid(), from: braaStart, to: { lat, lon } }])
  ...
  return (
    <div style={{ position: 'relative', flex: 1, overflow: 'hidden', display: 'flex' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '10px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                ...
                {watches.size > 0 && (
                  <button onClick={() => setWatches(new Set())}
                    style={{
                      marginTop: '4px', background: 'none', border: `1px solid ${HUD_BORDER}`,
            ...
            {consoleTab === 'draw' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                ...
                {braaStart && (
                  <div style={{
                    padding: '5px 7px', borderRadius: '2px', background: 'rgba(255,200,0,0.08)',
                ...
                {drawnBraas.length > 0 && (
                  <button onClick={() => setDrawnBraas([])}
                    style={{
                      background: 'none', border: `1px solid ${HUD_BORDER}`,
                      borderRadius: '2px', color: HUD_DIM, cursor: 'pointer', padding: '4px',
                      fontFamily: FONT_HEAD, fontSize: '0.65rem', letterSpacing: '0.12em'
                    }}>
                    CLEAR ALL
      ...
      <div style={{ position: 'relative', flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{ ...hudPanel, display: 'flex', gap: '2px', flexWrap: 'wrap', justifyContent: 'flex-end', padding: '4px 9px' }}>
            {DCS_REGIONS.map(r => (
              <button key={r.label} onClick={() => setFlyTarget({ center: r.center, zoom: r.zoom })}
                style={{
                  fontFamily: FONT_HEAD, fontSize: '0.62rem', letterSpacing: '0.1em',
        ...
        {isLoading && (
          <div style={{
            position: 'absolute', inset: 0, zIndex: 2000, display: 'flex', alignItems: 'center',
            justifyContent: 'center', background: 'rgba(6,10,6,0.8)', pointerEvents: 'none'
          }}>
            <span style={{
              fontFamily: FONT_HEAD, fontSize: '1.1rem', letterSpacing: '0.3em', color: GREEN,
        ...
        <div style={{
          ...
          <div style={{ padding: '9px 13px', borderBottom: `1px solid ${HUD_BORDER}` }}>
            ...
            <input type="number" min={0} max={2000} value={speed} onChange={e => setSpeed(Number(e.target.value))}
              style={{
                width: '100%', background: 'rgba(0,20,0,0.5)', border: `1px solid ${HUD_BORDER}`,
