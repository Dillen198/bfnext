...
export default function KillFeed() {
  const { data: kills = [], isLoading, dataUpdatedAt } = useQuery({
    queryKey: ['kills-feed'],
    queryFn: () => api.kills(undefined, 150),
      ...
      <div className="flex-1 overflow-auto p-4 space-y-3 grid-bg">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={s.label} className="tac-card p-4" style={{ borderColor: `${s.color}22` } as React.CSSProperties}>
              <div className="absolute inset-x-0 top-0 h-px" style={{ background: `linear-gradient(to right,transparent,${s.color}55,transparent)` }} />
              <div className="flex items-center justify-between mb-2">
                <span className="text-[9px] text-slate-600 uppercase tracking-widest">{s.label}</span>
                <s.icon size={11} style={{ color: s.color }} />
              </div>
              <div className="text-[26px] leading-none font-bold font-mono tabular-nums" style={{ color: s.color }}>{s.value}</div>
            </div>
          ))}
        </div>
          ...
          <Card className="lg:col-span-2">
            ...
            <div className="p-4">
              {topWeapons.length > 0 ? (
                ...
                <div className="h-[130px] flex items-center justify-center text-slate-700 text-xs">No data</div>
              )}
            </div>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader
            ...
            icon={Crosshair}
            color="text-red-400"
            right={<span className="text-[9px] text-slate-700 font-mono">{kills.length} events</span>}
          />
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {isLoading && (
                  <tr><td colSpan={6} className="text-center py-10 text-slate-700 text-xs">Loading…</td></tr>
                )}
                {kills.length === 0 && !isLoading && (
                  <tr><td colSpan={6} className="text-center py-10 text-slate-700 text-xs">No kills recorded yet</td></tr>
                )}
                {kills.map((k, i) => {
                  const killerName = k.killer?.ucid ? (nameMap.get(k.killer.ucid) ?? '???') : null
                  const victimName = k.victim.ucid ? (nameMap.get(k.victim.ucid) ?? '???') : null
                  const kt = classifyKill(k.target_type, k.killer?.weapon ?? null)
                  return (
                    <tr key={i} className="border-b border-[#080f1b] kill-row">
                      <td className="px-3 py-2 text-[10px] font-mono text-slate-600 whitespace-nowrap tabular-nums">
                        {new Date(k.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                      </td>
                      <td className="px-3 py-2">
                        <span className="text-[9px] font-bold px-1.5 py-0.5 rounded" style={{ color: kt.color, background: `${kt.color}18` }}>
                          {kt.label}
                        </span>
                      </td>
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[11px] text-slate-200 font-semibold whitespace-nowrap">
                            {killerName ?? <span className="text-slate-600 font-normal">AI</span>}
                          </span>
                        </div>
                      </td>
                      <td className="px-3 py-2">
                        <span className="text-[10px] font-mono text-yellow-500/80 whitespace-nowrap">
                      ...
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-2">
                          ...
                          {victimName && <span className="text-[10px] text-slate-400">{victimName}</span>}
                        </div>
                      </td>
                    </tr>
                  )
