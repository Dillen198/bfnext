import ms, { type ColorMode as MsColorMode, type Symbol as MsSymbol } from "milsymbol";

import { AircraftToSidcIcon } from "./dcs/aircraft";
import { type Settings } from "./settings";
import { type TacviewObject } from "./tacview";

export const colorMode: MsColorMode = ms.ColorMode(
  "#ffffff",
  "#17c2f6",
  "#ff8080",
  "#FDE68A",
  "#ffffff",
  // milsymbol v3 added a 6th (Suspect) slot; peace-eye targeted v2's 5-arg form.
  "#ff8080"
);
const symbolCache: Record<string, MsSymbol> = {};
...
export function sidcToSymbol(sidc: string): MsSymbol {
  if (symbolCache[sidc] != null) {
    return symbolCache[sidc];
  } else {
    // milsymbol v3's `colorMode` only tints the fill; for peace-eye's
    // fill:false framed symbols we recolour the whole glyph via `monoColor`,
    // keyed off the SIDC standard-identity digit (index 3).
    const ident = sidc.charAt(3);
    const mono =
      ident === "3" ? colorMode.Friend
      : ident === "6" ? colorMode.Hostile
      : colorMode.Unknown;
...
export function objectToSymbol(object: TacviewObject): MsSymbol {
