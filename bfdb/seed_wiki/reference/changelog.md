...
When enabled, the line dividing blue-held ground from red-held ground is drawn as a white dashed line. It's the zero line of a smooth influence field — every owned objective pulls the ground toward its side, with the pull fading over distance — so it follows the overall shape of the theatre and ignores a lone base sitting behind enemy lines. A theatre with an island and two land borders shows up as several distinct fronts; nothing is drawn where one side holds everything. It redraws whenever an objective changes hands.
